...
#include "XrdEc/v2zip/XrdEcWrtBuff.hh"
#include "XrdEc/v2zip/XrdEcZipUtilities.hh"
  ...
  class DataObject
  {
    public:
      ...
      inline void Open( const ObjCfg &objcfg, OpenMode mode, XrdCl::ResponseHandler *handler )
      {
        ...
        if( mode == OpenMode::ReadMode ) OpenForRead( handler );
        else if( mode == OpenMode::WriteMode )
        {
      ...
      {
        if( mode == OpenMode::ReadMode ) CloseAfterRead( handler );
        else if( mode == OpenMode::WriteMode ) CloseAfterWrite( handler );
      }
