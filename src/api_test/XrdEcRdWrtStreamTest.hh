/*
 * XrdEcRdWrtStreamTest.hh
 *
 *  Created on: Aug 25, 2017
 *      Author: simonm
 */

#ifndef SRC_API_TEST_XRDECRDWRTSTREAMTEST_HH_
#define SRC_API_TEST_XRDECRDWRTSTREAMTEST_HH_
...
//
////  XrdEc::RdWrtCache cache( "/first_test.txt" );
//  XrdEc::RdWrtStream stream( "/first_test.txt", 4 );
//
...
//    size_t bytesWritten = stream.Write( offset, size, buffer );
//    buffer += bytesWritten;
//    offset += bytesWritten;
//
//    int ocasional_read = distribution( generator );
//    if( ocasional_read > 50 )
//    {
//      size_t rdOff  = size_dist( generator ) % offset;
//      size_t rdSize = size_dist( generator ) % ( offset - rdOff );
//
//      char *rdBuff = new char[rdSize];
//      size_t bytesRd = stream.Read( rdOff, rdSize, rdBuff );
//      std::string str( rdBuff, bytesRd );
//      if( rdSize != bytesRd )
//      {
//        std::cout << "Size : " << ( bytesRd == rdSize ? "TRUE" : "FALSE") << std::endl;
//        std::cout << rdSize << std::endl;
//        std::cout << bytesRd << std::endl;
//        std::cout << rdOff << std::endl;
//        std::cout << offset << std::endl;
//      }
//      std::cout << "Occasional read : " << ( str == input.substr( rdOff, bytesRd ) ? "TRUE" : "FALSE" ) << std::endl;
//      delete[] rdBuff;
//    }
//  }
//  stream.Sync();
...
//  std::cout << stream.GetLBI() << std::endl;
//
//  std::string str;
//  char buff[1024];
//  offset = 0;
...
//      size = input.size() - offset;
//    offset += stream.Read( offset, size, buff );
//    str.append( buff, size );
//  }
//
//  std::cout << str.size() << std::endl;
//  std::cout << input.size() << std::endl;
//  std::cout << "equals : " << ( input == str ? "TRUE" : "FALSE" ) << std::endl;
//
//  auto p = std::mismatch( input.begin(), input.end(), str.begin() );
//  if( p.first != input.end() && p.second != str.end() )
//  {
//    std::cout << "mismatch : " << ( p.first - input.begin() ) << std::endl;
//    std::cout << std::string( &(*p.first), 1024 ) << std::endl;
//    std::cout << std::string( &(*p.second), 1024 ) << std::endl;
//  }
//  stream.Sync();


#endif /* SRC_API_TEST_XRDECRDWRTSTREAMTEST_HH_ */
